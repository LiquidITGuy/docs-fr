...
Active TLS + HTTP/2. Notez que cela est downgradé à TLS uniquement lorsque l'[option `server.proxy`](./server-options#server-proxy) est également utilisée.

La valeur peut également être un [objet d'options](https://nodejs.org/api/https.html#https_https_createserver_options_requestlistener) passé à `https.createServer()`.
